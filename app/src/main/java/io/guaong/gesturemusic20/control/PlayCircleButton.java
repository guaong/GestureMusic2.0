...
public class PlayCircleButton extends CircleButton {
    ...
    public PlayCircleButton(Context context) {
        super(context);
    }

    public PlayCircleButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        mPaint = new Paint();
        // 去除画笔锯齿
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.AT_MOST || heightMode == MeasureSpec.UNSPECIFIED){
            heightSize = WindowUtil.getWindowHeight(getContext()) / 2;
        }
        if (widthMode == MeasureSpec.AT_MOST || widthMode == MeasureSpec.UNSPECIFIED){
            widthSize = WindowUtil.getWindowWidth(getContext());
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        rX = w / 2;
        rY = h / 2;
        left = rX - (float)(Math.cos(Math.PI / 3) * r * 0.8f);
        right = rX + (float)(Math.cos(Math.PI / 3) * r * 0.8f);
        top = rY - (float)(Math.sin(Math.PI / 3) * r * 0.8f);
        bottom = rY + (float)(Math.sin(Math.PI / 3) * r * 0.8f);
        rightCenter = r * 0.8f + rX;
